 ...
 *
 * Note: This handler is specific to Kitty terminal. It gracefully does
 * nothing if Kitty is not detected.
 */

import { isValidVoiceCompletion, getTabFallback } from '../lib/response-format';
import type { ParsedTranscript, ResponseState } from '../tools/TranscriptParser';
...
export async function handleTabState(parsed: ParsedTranscript): Promise<void> {
  // Skip if not in Kitty terminal
  if (!isKitty()) {
    console.error('[TabState] Not in Kitty terminal, skipping');
    return;
  }
