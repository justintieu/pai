...
export function categorizeChange(path: string): ChangeCategory | null {
  ...
  for (const excluded of EXCLUDED_PATHS) {
    if (path.toLowerCase().includes(excluded)) {
      return null;
    }
  }
  ...
  if (path.includes('context/')) return 'core-system';
...
// 2 minutes cooldown between integrity checks
...
export function generateDescriptiveTitle(changes: FileChange[]): string {
  ...
  const hasContext = paths.some(p => p.includes('context/'));
  ...
  // Context changes
    ...
    const contextFiles = paths
      .filter(p => p.includes('context/'))
      .map(p => basename(p, '.md'));
    if (contextFiles.length === 1) {
      title = `${contextFiles[0]} Context Updated`;
    } else {
      title = 'Context Configuration Updated';
    }
  }
  // Fallback
  else {
