 ...
 * - Voice: ElevenLabs TTS (optional, graceful degradation)
 *
 * Design principles:
...
export interface NotificationConfig {
  ...
  voice: {
    enabled: boolean;
    serverUrl: string;  // Voice server URL (default: localhost:8888)
  };
  thresholds: {
    longTaskMinutes: number;
  };
  routing: {
    [key in NotificationEvent]: ('ntfy' | 'discord' | 'desktop' | 'sms' | 'voice')[];
  };
}
...
const DEFAULT_CONFIG: NotificationConfig = {
  ...
  voice: {
    enabled: false,  // Disabled by default - requires ElevenLabs API key
  ...
  routing: {
    taskComplete: ['voice'],              // Voice only for normal tasks
    longTask: ['ntfy', 'voice'],          // Push + voice for long tasks
    backgroundAgent: ['ntfy', 'voice'],   // Push + voice for background completions
    error: ['ntfy', 'discord', 'voice'],  // Multiple channels for errors
    security: ['ntfy', 'discord', 'sms']  // All channels for security (no voice)
  }
};
    ...
    if (existsSync(settingsPath)) {
      ...
      if (settings.notifications) {
        return {
          ...
          voice: { ...DEFAULT_CONFIG.voice, ...settings.notifications?.voice },
...
/**
 * Send voice notification via voice server (ElevenLabs)
 * Gracefully degrades if server is not running or no API key configured
 */
export async function sendVoice(
  ...
  try {
    ...
    const payload = {
      ...
      voice_id: options.voiceId || identity.voiceId,
    };

    const response = await fetch(`${config.voice.serverUrl}/notify`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify(payload),
    });
    ...
    // Graceful degradation - voice server not running
    // This is expected if user hasn't set up ElevenLabs
    return false;
  }
}
...
export async function notify(
  ...
  for (const channel of channels) {
    switch (channel) {
      ...
      case 'voice':
        promises.push(sendVoice(message, {
