...
const CORE_SKILL_PATH = paiPath('skills', 'CORE', 'SKILL.md');
 ...
 * Tab summaries should start with a gerund (e.g., "Fixing bug", "Checking config")
 */
 ...
 * MUST return gerunds to pass isValidTabSummary validation
 */
export function getTabFallback(stage: 'start' | 'end' = 'start'): string {
  ...
  // MUST be gerunds (ending in "ing") to pass validation
  return stage === 'end' ? 'Finishing task' : 'Processing request';
}
