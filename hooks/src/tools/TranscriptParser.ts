  ...
  for (const line of lines) {
    if (line.trim()) {
      try {
        ...
        if (entry.type === 'assistant' && (entry.message as Record<string, unknown>)?.content) {
...
export function detectResponseState(lastMessage: string, transcriptContent: string): ResponseState {
  try {
    ...
    const lines = transcriptContent.trim().split('\n');
    let lastAssistantEntry: Record<string, unknown> | null = null;
    ...
    if (lastAssistantEntry?.message) {
      const message = lastAssistantEntry.message as Record<string, unknown>;
      const content = Array.isArray(message.content) ? message.content : [];
      for (const block of content) {
        const typedBlock = block as Record<string, unknown>;
        if (typedBlock.type === 'tool_use' && typedBlock.name === 'AskUserQuestion') {
